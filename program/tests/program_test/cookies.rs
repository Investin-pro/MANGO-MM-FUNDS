...
pub struct MangoGroupCookie {
    ...
    pub investin_funds: Vec<InvestinFundCookie>,
...
impl MangoGroupCookie {
    ...
    pub async fn default(test: &mut MangoProgramTest) -> Self {
        ...
        let quote_mint_pk
         = test.mints[test.quote_index].pubkey.unwrap();
        ...
        let quote_optimal_util = I80F48::from_num(0.7 as f64);
    ...
    pub async fn full_setup(
        ...
        self.investin_funds = self.add_investin_funds(test, num_users).await;
    ...
    #[allow(dead_code)]
    pub async fn add_investin_funds(
        &mut self,
        test: &mut MangoProgramTest,
        num_users: usize,
    ) -> Vec<InvestinFundCookie> {
        let mut investin_funds = Vec::new();
        for i in 0..num_users {
            investin_funds.push(InvestinFundCookie::init(test, self, i).await);
        }
        investin_funds
    }
...
pub struct InvestinFundCookie {
    pub fund_address: Pubkey,
    pub manager: Keypair,
    pub fund_data: FundData,
}

impl InvestinFundCookie {
    #[allow(dead_code)]
    pub async fn init(
        test: &mut MangoProgramTest,
        mango_group_cookie: &mut MangoGroupCookie,
        ...
        let mango_program_id = test.mango_program_id;
        let investin_program_id = test.investin_program_id;
        let manager = Keypair::from_base58_string(&test.users[user_index].to_base58_string());
        let manager_pk = manager.pubkey();

        let investin_seeds: &[&[u8]] = &[&manager_pk.to_bytes()];
        ...
        let instructions = [mm::instruction::initialize(
            ...
            500)
        .unwrap()];
        test.process_transaction(&instructions, Some(&[&manager])).await.unwrap();
        let fund_data = test.load_investin_account::<FundData>(investin_fund_pk).await;
        InvestinFundCookie { fund_address: investin_fund_pk, manager, fund_data}
    }
}
