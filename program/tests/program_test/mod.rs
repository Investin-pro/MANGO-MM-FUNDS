...
use investin::processor::{usdc_token, mango_v3};
...
use mango::{ids::*, instruction::*, matching::*, oracle::*, state::*, utils::*};
use mango_common::Loadable;
use investin::{entrypoint::*, instruction::*, state::*};
...
pub struct MangoProgramTest {
    ...
    pub rent: Rent,
    pub investin_program_id: Pubkey,
...
impl MangoProgramTest {
    ...
    pub async fn load_investin_account<T: investin::state::Loadable>(&mut self, acc_pk: Pubkey) -> T {
        let acc = self.context.banks_client.get_account(acc_pk).await.unwrap().unwrap();
        // let acc_info: AccountInfo = (&acc_pk, &mut acc).into();
        return *T::load_from_bytes(&acc.data).unwrap();
    }
